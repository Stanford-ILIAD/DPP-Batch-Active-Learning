...
import algos
from models import LDS, Driver, LunarLander, MountainCar, Swimmer, Tosser


def get_feedback(simulation_object, input_A, input_B, w_true):
    ...
    # Uncomment below to enable visualizations and user selections, and see the next comment
    '''
    s = 0
    while s==0:
        ...
        if selection == 'a':
            ...
            s = -1
    '''
	# ... (continued) and comment below 4 lines
    if np.dot(w_true,phi_A) > np.dot(w_true,phi_B):
        s = 1
    else:
        s = -1
    return psi, s
